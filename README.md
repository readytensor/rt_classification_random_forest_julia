# Random Forest Classification Model in Julia

This is a dockerized, Julia-based Random Forest model that can be used for binary and multiclass classification tasks. The purpose of this project is to provide a functional model that can be used as a template for building your own classification model.

The implementation contains a simple preprocessing pipeline which handles missing data and categorical features. The main prediction model is Random Forest. You may replace the preprocessing logic, or the Random Forest model, with your own custom logic. The model is implemented in Julia scripts for training and batch prediction tasks.
